...
BuildRequires:  %{?scl_prefix}tbb-devel
# build inside collection
%{?scl:BuildRequires: %{?scl_prefix}python-devel}
# runtime-depend on collection
%{?scl:Requires: %{scl}-runtime}
Obsoletes:      %{?scl_prefix}umfpack <= 5.0.1
Obsoletes:      %{?scl_prefix}ufsparse <= 2.1.1
Provides:       %{?scl_prefix}ufsparse = %{version}-%{release}
...
Obsoletes:      %{?scl_prefix}umfpack-devel <= 5.0.1
Obsoletes:      %{?scl_prefix}ufsparse-devel <= 2.1.1
Provides:       %{?scl_prefix}ufsparse-devel = %{version}-%{release}
...
Provides:       %{?scl_prefix}ufsparse-static = %{version}-%{release}
...
%description doc
This package contains documentation files for %{pkg_name}.
...
pushd CHOLMOD
  ...
  pushd ../Lib
    gcc -shared -Wl,-soname,libcholmod.so.%{cholmod_version_major} -o \
        ...
        -L%{_root_libdir}/atlas %{atlaslibs} \
...
pushd SPQR
  pushd Lib
    make CFLAGS="$RPM_OPT_FLAGS %{?with_tbb:-DHAVE_TBB} -DNPARTITION -fPIC %{?scl:-I%{_includedir}}"
  popd
  pushd ../Lib
    g++ -shared -Wl,-soname,libspqr.so.%{spqr_version_major} -o \
        libspqr.so.%{spqr_version} ../SPQR/Lib/*.o \
        -L%{_root_libdir}/atlas %{atlaslibs} \
        %{?with_tbb:-L%{_libdir} -ltbb -ltbbmalloc} \
        libcholmod.so.%{cholmod_version_major} \
        libsuitesparseconfig.so.%{SuiteSparse_config_major} -lm
...
pushd Include
  for f in *.h *.hpp;  do
    cp -a $f ${RPM_BUILD_ROOT}%{_includedir}/%{pkg_name}/$f
  done
popd

# create license_texts directory for inclusion in all subpackages
...
%changelog
* Mon Mar 31 2014 Tomas Tomecek <ttomecek@redhat.com> - 4.0.2-13
- explicitly require SCL runtime

* Thu Mar 20 2014 Tomas Tomecek <ttomecek@redhat.com> - 4.0.2-12
- rebuilt with latest scl-utils (dependencies problems)

* Thu Feb 20 2014 Bohuslav Kabrda <bkabrda@redhat.com> - 4.0.2-11
- add licenses to all subpackages
Resolves: rhbz#1066039

* Tue Jan 28 2014 Tomas Tomecek <ttomecek@redhat.com> - 4.0.2-10
- fix debuginfo conflicts (rhbz#1058834)

* Fri Nov 15 2013 Tomas Tomecek <ttomecek@redhat.com> - 4.0.2-9
- link against tbb properly

* Thu Nov 14 2013 Tomas Tomecek <ttomecek@redhat.com> - 4.0.2-8
- initial RHSCL-1.1 build
